...
`account_name` - швидкий спосіб прив'язати рядок до одного акаунта. Можна вказувати або внутрішній `name` з account.json, або TikTok `username` цього акаунта.

`allowed_accounts` або `eligible_accounts` - список назв акаунтів через `|` або кому, яким дозволено надсилати цей коментар.

`target_username` - юзернейм автора цільового відео без `@`. Якщо вказати, бот публікує коментар тільки коли відкрито сторінку потрібного автора.
